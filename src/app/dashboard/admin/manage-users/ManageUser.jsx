...
export default function ManageUsers() {
  ...
  const deleteUser = async (userId) => {
    ...
    setActionLoading(userId);
    try {
      const { error } = await authClient.admin.deleteUser({
        userId,
      });
        ...
        <table className="table w-full">
          ...
          <tbody>
            {users.length === 0 ? (
              ...
              users.map((user) => (
                <tr key={user._id} className="border-b border-base-300 hover:bg-base-100">
                  <td className="font-medium">{user.name || "—"}</td>
                  <td className="font-mono text-sm">{user.email}</td>
                  <td>
                    <span className={`badge ${
                      user.role === 'admin' ? 'badge-error' : 
                      user.role === 'librarian' ? 'badge-warning' : 'badge-success'
                    }`}>
                      {user.role || "user"}
                    </span>
                  </td>
